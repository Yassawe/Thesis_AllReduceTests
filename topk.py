...
def runProcess(rank, args):
    ...
    for i in range(1000):
        shrinked_data, idx = torch.topk(data, k=int(args.top*args.M), sorted=False)
        dataWithIdx = torch.cat([shrinked_data, idx])
        dist.all_reduce(dataWithIdx, op=dist.ReduceOp.SUM)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--std", default = 0.00511206 , type=float)
    parser.add_argument("--top", default = 0.1, type=float)
